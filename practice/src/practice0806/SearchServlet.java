...
public class SearchServlet extends HttpServlet {
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i=0;i<list.size();i++) {
			...
			out.print("<tr><td>" + id + "</td><td>" + name + "</td><td>" + address + "</td><td>" + "<a href='/practice/search2?command=searchOrder&searchid=" + id + "'> 주문조회 </a></td>" + "<td><a href='/practice/search?command=delMember&delid=" + id + "&searchid=" + id2 + "'> 삭제 </a></td></tr>");

		}
		
		out.print("</table></body></html>");
		out.print("<a href='/practice/search.html'>새로운 고객 조회하기</a>");
	}
