...
//			pstmt.setString(1, vo.getSearchId());
	...
	public void delete(CustomerVO vo) throws SQLException {
		PreparedStatement pstmt = null;
	...
	public void before(CustomerVO vo) throws SQLException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			con = dataFactory.getConnection();
			...
			while (rs.next()) {
				before_id = rs.getString("before");
			}
			
			if(before_id == null) {
				before_id = vo.getId();
			}
			vo.setSearchId(before_id);
	...
	public void after(CustomerVO vo) throws SQLException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			con = dataFactory.getConnection();
			...
			while (rs.next()) {
				after_id = rs.getString("after");
			}
			if(after_id == null) {
				after_id = vo.getId();
			}
			
			vo.setSearchId(after_id);
